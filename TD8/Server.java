...
public class Server {
	 ...
	 *
	 * @param bytes L'ensemble d'octets à envoyer.
	 */
	...
	/**
	 * Envoi un nombre donné d'octets sur le flux sortant du serveur.
	 *
	 * Cette methode est à utiliser lorsqu'on souhaite transférer un nombre
	 * donné d'octets qui ne sont pas affichables.
	 *
	 * @param bytes Les octets à envoyer.
	 * @param length Le nombre d'octets à envoyer.
	 */
	public void write(byte[] bytes, int length) throws IOException {
		// Ecriture du nombre donné d'octets sur le flux sortant du serveur.
		new DataOutputStream(client.getOutputStream()).write(bytes, 0, length);
	}
